...
- `examples/go-backend/` — the scaffold with a Go process behind it; the Go host itself is [`rsc-kit/go`](https://github.com/rsc-kit/go), and `packages/core/tests/fixtures/go-host` is the engine's test server on it
...
rscKit()` onward in `vite.ts` is a **template literal**. A helper added there
...
`rscKit()` therefore returns a `define` for it. The rsc and ssr bundles go
...
`rscKit()` resolves `@vitejs/plugin-rsc` against the project root and
...
what the build just produced. `rscKit()` is ordered after
...
`rscKit()` is published on its own, so it defaults to nothing a particular
...
(`rscKit({ sourceDir: 'resources/js/rsc' })`), not in `config/rsc.php`.
