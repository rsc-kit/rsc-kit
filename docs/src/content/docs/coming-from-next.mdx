...
| `Viewport` from `next`, `export const viewport` | the same export, typed from `@rsc-kit/core/metadata`; the charset and viewport tags Next wrote for you are written here too — [Metadata](/guides/metadata#charset-and-viewport) |
| `cookies().set()` in an action, then `revalidatePath()` | the same: a cookie set is what `cookies().get()` answers for the rest of the request, the revalidated sections included |
...
| a hand-written OpenAPI spec, `@scalar/nextjs-api-reference` | `rscKit({ openapi })` derives the document from every `route.ts` and its schemas; the page is Scalar's renderer, `@scalar/client-side-rendering`, in a `route.ts` — the Next package works too but installs Next with it — [OpenAPI](/guides/openapi) |
...
default like React Hook Form's `register`, and read with two words:
`error('title')` for a field's error, `field('title')` for the rare field whose
value you hold — the same two in a child component through `useForm()`.
shadcn's `Field` components fit as they are. See [Forms](/guides/forms).
