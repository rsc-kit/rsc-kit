...
`onMessage` hands every message to a store that already holds the value, so
the query stays the source of truth and the stream nudges it — the pattern
TanStack's own docs describe, without the `EventSource` plumbing:
