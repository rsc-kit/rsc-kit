...
A request carrying a **query string** also runs the route. The build answered
the bare url, and a route that reads `?limit=` answers differently for every
value — so rather than guessing which ones do, anything with a query goes to
the route itself.
