...
A page rendered per request is not one of these files, and says
`private, max-age=0, must-revalidate`: it may have read the visitor's cookies,
and its headers leave before the render could say whether it did.
...
**Do not blanket-cache the whole site.** A route with middleware — a
`middleware.ts`, or the host's `export const middleware = ['auth']` — is
deliberately sent as:
