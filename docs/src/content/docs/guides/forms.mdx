...
That is a real trade and it is the right one for the number of controlled fields
a form usually has — one or two, for a character count or a control with no
native element. What their design also gives is per-field meta, and that needed
no render prop at all: it is `fieldState()`.

:::note[Where this gives out]
Bind thirty fields and typing in one re-renders all thirty, because they share
this component's state. `<Controller>` would not. If you are writing a form that
large and that controlled, reach for react-hook-form — nothing here is trying to
be a form library.
:::
