...
The brackets are dropped from the key. It is also what `useForm` writes when it
serialises an array, so a value survives a trip through both.

:::note[Not nested field arrays]
This is a flat list of values. A repeating *group* — `items[0].name`,
`items[1].name` — is not parsed into nested objects; those keys arrive as
written. If you need that shape, hold it in state with `useForm` and send it
through `transform`.
:::
...
`<Form>` unless you need to own the values. It covers most forms, and it is the
one that works before hydration — `useForm` cannot, because there is no form
element until React has run.

Reach for `useForm` when the DOM cannot hold the value for you:

- a component with no native control behind it — a rich text editor, a map
  picker, a drag-to-reorder list
- something that reads the value as it is typed, like a character count
- a repeating group you add and remove rows from
- a submit that happens somewhere other than a form — a toolbar button, a step
  in a wizard

Both take the same `schema`, report the same server errors, and cost nothing if
unused: they are separate entry points, so importing one does not pull in the
other.
