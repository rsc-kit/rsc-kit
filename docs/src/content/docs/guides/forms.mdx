...
Both hooks read a context, so they work anywhere below `<Form>` — which is
usually enough, because the `<form>` element can wrap as much of the page as you
like.

A component that is genuinely a sibling cannot see in. react-hook-form and
TanStack Form do not have this limit because their state lives in a hook *you*
call, above the form. Here it lives in the form. If you need values outside,
lift them: keep your own state and update it from `onChange`.
:::
