...
like the more capable design, and there is a specific reason they need it that
does not apply here.

Both are **controlled-first**: every value lives in the form's state, so without
per-field subscriptions one keystroke re-renders every field. The render prop is
what scopes that. The verbosity is the price of the performance.

Here, most fields are not in state at all — they are uncontrolled, and typing in
one re-renders nothing. Only the fields you bind with `field()` participate, and
you bind the ones that need it. So the problem the render prop solves mostly
does not arise, and paying its cost on every field would be paying for nothing.

What their design genuinely gives is per-field meta, and that needed no render
prop — it is `fieldState()`.

:::note[Where this gives out]
Bind thirty fields and typing in one will re-render all thirty, because they
share this component's state. That is the case their architecture is built for.
If you are writing a form that large and fully controlled, reach for one of
them; nothing here is trying to be a form library.
:::
