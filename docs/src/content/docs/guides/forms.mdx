...
Laravel RSC provides a `<Form>` component and `useForm` hook for handling form submissions with server actions. Built-in support for validation errors, loading states, optimistic updates, and GET form navigation.
...
import { Form } from "@rsc-router/core/form";
...
import { useFormStatus } from "@rsc-router/core/form";
...
import { useForm } from "@rsc-router/core/form";
...
When a server action throws a Laravel `ValidationException`, Laravel RSC catches it and returns a 422 response. Both `<Form>` and `useForm` automatically populate the `errors` object — no manual error catching needed.

```php title="app/Rsc/Actions/CreateTodo.php"
<?php
...
Errors are typed as `Partial<Record<keyof T, string[]>>` — each field maps to an array of error messages, matching Laravel's error bag format.
