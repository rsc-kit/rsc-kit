...
`field()` covers a control that holds its own value and a value read as it is
typed, so those are no longer a reason to leave.

What is left for `useForm` is a form with no `<form>` in it:

- a submit from somewhere else entirely — a toolbar button, a step in a wizard
- rows you add and remove, where the number of fields is state rather than
  markup
- a value that has to be transformed before it is sent, through `transform`

Both take the same `schema` and report the same server errors, and cost nothing
if unused: they are separate entry points, so importing one does not pull in
the other.
