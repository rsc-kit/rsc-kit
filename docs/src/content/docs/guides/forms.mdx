...
`<Form>`. It does everything `useForm` does now, and one thing `useForm`
cannot: work before hydration.

`useForm` is **deprecated**. It was the only way to control a field, read a
value as it was typed, or know a submit had just succeeded — and those are
`field()`, `field(name).value` and `succeeded` now.

The last reason to reach for it was a submit from outside the form, and that is
an HTML feature rather than a reason for a second API:
...
It still works and is not going anywhere before 1.0 — it costs nothing when it
is not imported, and breaking a published API to remove a duplicate is churn.
New forms should use `<Form>`.
