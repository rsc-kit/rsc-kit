...
branch its discriminator names; a text input left blank is absent for a
string the schema does not require, so `z.email().optional()` accepts the
empty field, and is `""` for one it requires, so `z.string().min(1)` can say
so. A value the schema refuses is still refused as itself — `"many"` for a
