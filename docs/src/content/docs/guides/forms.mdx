...
it returned as React's form state — a `useActionState` form shows its
result. A `redirect()` the action throws is followed as a document's would
be; a cookie it sets is on the answer. Same origin only, as an action is,
and the answer is never stored. A stored page is the one most likely to be
submitted this way — it paints before its runtime arrives — which is why
