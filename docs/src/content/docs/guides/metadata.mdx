...
`appleWebApp` is Next's shape — `capable`, `title`, `statusBarStyle`,
`startupImage` — and renders the same tags. Launch screens are simpler as files
named by their size; see [Splash screens](/guides/pwa#splash-screens).

```tsx title="src/app/layout.tsx"
export const metadata: Metadata = {
  appleWebApp: { capable: true, title: 'Orders', statusBarStyle: 'black-translucent' },
}
```
