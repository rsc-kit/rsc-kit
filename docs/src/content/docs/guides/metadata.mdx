...
:::note[`og:image` wants an absolute url]
The OpenGraph spec asks for one and several crawlers mean it. The build emits a
relative path unless it knows your site's origin, which is better than omitting
the tag but not as good as the real thing — set `og:image` yourself in
`metadata` when you know the domain.
:::
