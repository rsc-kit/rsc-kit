...
browser offer to put it on a home screen — that needs a web app manifest, and
it is a separate option because they are separate decisions:
...
The build writes `manifest.webmanifest` and links it from every page. There is
no layout to edit — React hoists the `<link>` and the `theme-color` meta into
`<head>` from wherever they are rendered, so this works on an app that already
...
An installable app, not a full PWA. There is no push, no background sync, and
the service worker precaches the built assets rather than your pages — so a
visited page is not yet available offline, only the shell that renders it.
