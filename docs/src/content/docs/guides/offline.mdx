...
an answer that was narrowed by the visitor — a guarded page, or anything that
read the session — would be handed to whoever opens the app next, on the same
machine, signed in as someone else. `no-store` is the server saying that must
not happen, and the worker treats it as an instruction rather than a hint.

A server function is a POST and is never cached here either, which is worth
knowing before designing around this: data a client component fetches for
itself does not survive a reload, however the page that shows it was served. To
keep a visitor's own data across reloads, persist your cache library — SWR and
TanStack Query both write to storage that belongs to that browser, which gets
you the result without putting one visitor's data where the next one can be
served it.
