...
- **`usePathname()`** in a client component, for the same route: one shell
  serves every url it matches, so the hook throws during that render and the
  component lands in its nearest `<Suspense>` fallback, rendered by the
  browser with the real url. Above every boundary, the route is refused with
  the hook named. Where the url is known — a listed url, a request — the
  hook answers as always.
