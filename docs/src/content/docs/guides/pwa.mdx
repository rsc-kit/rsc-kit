...
### A dark splash, and the frame after it

Between the splash and the page's first paint, iOS fills the screen with the
web view's own background. That fill is white unless the page says otherwise.
With a dark splash and a dark theme, that is a white flash on every launch in
dark mode only; in light mode it goes unnoticed.

Two lines fix it: declare the schemes the page supports, so the fill follows
the phone's theme before any stylesheet has loaded, and give `html` the
page's background so overscroll and the frames before the body paints match.
...
The second rule is for a class-based theme (`next-themes` with
`attribute="class"`): once the stylesheet has loaded, the scheme follows the
theme the visitor chose rather than the system's.
