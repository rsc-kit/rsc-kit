...
A value that keeps changing while someone watches has two transports, and
neither is part of `query()`, which answers once and is cacheable.

**Polling** reads the query again on an interval. It reuses what you already
wrote, needs no special route, and goes through the query's `Cache-Control` —
a thousand tabs polling the same seat count become one origin request per
interval at the CDN. It pauses while the tab is hidden and never overlaps two
reads.
...
polling that stops on its own. `until` says when a read is the last one;
`onSettled` fires once, on that read; `settled` resolves with it. The result
is the data, and what to do when it settles is the page's to decide — so the
same primitive serves two pages that want different things:
...
**Server-sent events** push. Bytes go out only when something changed and
arrive at once, which is better per update by a wide margin. The costs are a
connection held open per tab for as long as it is open — fine on Bun or Node,
a limit on a Worker or a small container behind a balancer — no caching, and a
source of change to yield from: a database listener, a queue, a timer. A
generator that polls the database itself has only moved the polling and kept a
connection for the privilege.

Start with polling when you have no change feed yet or your host holds few
connections; move to events when you do and it does. The component reads the
same either way.

The route is an ordinary `route.ts`, so it lives beside its pages, runs the
`middleware.ts` above it, and takes the same params:
...
`events()` frames each yield as JSON, sends a keepalive so a proxy does not
drop an idle stream, and ends the generator when the browser goes away — that
is what `signal` is for. `yield { event: 'paid', id: '42', data }` names a
message or gives it an id, for a client that listens by name or resumes.
...
Both hooks hand every value to a store that already holds one, so the query
stays the source of truth and the stream nudges it — the pattern TanStack's
own docs describe, without the `EventSource` plumbing:
...
usePolling(() => fetchQuery(getSeats, []), { every: 2_000, onData: setSeats })
```
