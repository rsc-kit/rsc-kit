...
A read declared this way stops being labelled a write. Nothing else about the
call changes: same types, same arguments, same lack of an endpoint to write.

That matters in three places, most-likely-to-bite first:
...
`createServerFn()` is a GET by default and you opt into `{ method: 'POST' }` for
a mutation. We cannot copy that default, because `"use server"` already means
*action* in React and defaulting every registered function to GET would send
mutations as GET. So reads opt in, one at a time.
...
One caveat on the second bullet, so it is not oversold: data that is the same
for everyone usually wants [prerendering](/guides/static-generation/) rather
than a client read at all, and a personal read answers `no-store` and is not
cached either way. The caching win is real, and narrower than it sounds.
