...
The `"use server"` is still needed, and it is not saying the function is a
mutation. Despite the name, the directive means *this may be called from the
browser*: it is what makes React register the export and hand the client a stub
to call it through. Without it there is no id and no stub, and importing the
module from a client component would pull the real function body — and
everything it imports — into the browser bundle.

So the two markers answer different questions. `"use server"` says the function
crosses the boundary; `query()` says it is a read, and may be fetched as one.
...
a mutation. We cannot copy that default: React gives us one directive for
everything that crosses the boundary, so defaulting it to GET would send
mutations as GET. Reads opt in instead, one at a time.
