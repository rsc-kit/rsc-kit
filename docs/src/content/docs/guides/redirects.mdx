...
To the caller, the action **resolves** — with nothing — once the navigation
is under way. It does not throw: a `startTransition(async () => { await
logOut() })` with no `catch` around it would otherwise reject into React,
which unmounts the root, and a logout ended on a white page. There is
nothing to catch and nothing to check; a component that awaited the action
is on its way off the screen.
