...
**In a host's own middleware.** A backend that answers before this engine does
can redirect first. On Laravel that is `middleware: ['auth']` in a colocated
`route.ts`, named in Laravel's vocabulary and run through its pipeline.
