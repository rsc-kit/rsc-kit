...
The interceptor at `@modal/(.)photo/[id]/page.tsx` receives the target route's params. It can be a server component — only the chrome around it needs to be interactive:

```tsx title="app/@modal/(.)photo/[id]/page.tsx"
import { ModalShell } from '../../../components/ModalShell';

export default async function PhotoModal({ id }: { id: string }) {
  const photo = await rpc('photo', id);
...
A modal reachable from exactly one place can link to that place instead, and a real `<Link>` is worth keeping for it — a link can be opened in a new tab, and it still works before the page hydrates. The [demo](/intercept-demo) does that. `history.back()` is for a shell used from several.
...
[Open Intercept Demo →](/intercept-demo)

---

Route interception builds on [parallel routes](/rsc). Make sure you're familiar with the `@folder` convention before using interception patterns.

Next: [Typed Routes →](/typed-routes)
