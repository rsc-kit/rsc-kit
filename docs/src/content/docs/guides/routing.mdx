...
| `prefetch` | `"hover" \| "mount" \| "click" \| "none"` | `"hover"` | When to warm the cache. `hover` fetches the payload as the link comes into view, on every device, when the browser is idle, once per link — and decodes it, which loads the page's chunks, on intent: a pointer settled for 100 ms (so crossing a nav bar decodes nothing), a press, a touch. A click then finds everything there — [Navigation](/guides/navigation#what-a-click-finds-already-there). `true`/`false` are accepted as aliases for `hover`/`none`. |
