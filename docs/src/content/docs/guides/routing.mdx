...
| `prefetch` | `"hover" \| "mount" \| "click" \| "none"` | `"hover"` | When to warm the cache. `hover` waits 100 ms for the pointer to settle, so crossing a nav bar does not fire a request per link; on a device with no hover — a phone — it prefetches the link as it comes into view instead, when the browser is idle, once per link, which is what makes a tap land on a payload already there. `true`/`false` are accepted as aliases for `hover`/`none`. |
