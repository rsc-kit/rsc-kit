...
`?q=hats` — so there is no honest answer while a page is being stored, and
`useSearchParams` throws rather than pretending it is empty. A page rendered
for one request is different: the server knows that visitor's query string
and answers with it, as the dev server does for every page. The throw is only
ever the build's, and it lands in the nearest fallback.
