...
It throws, so nothing after it runs — a `return` you forget is not a bug you
can have. A string is one message; an array is several. The empty key is the
whole submission, for a refusal that is about no field in particular:

```ts
fieldErrors({ '': 'Sign-ups are closed for the weekend' });
```

:::note[Porting from next-safe-action]
`returnValidationErrors(schema, { email: { _errors: ['Account not found'] } })`
becomes `fieldErrors({ email: 'Account not found' })`. No schema argument,
because the typing comes from the handler; no `_errors` nesting; and no
`return` to forget.
:::
