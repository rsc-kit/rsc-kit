...
Three things follow from an action that revalidated, and none of them needs
code:

- **What is re-rendered replaces what is on screen in place.** `revalidate('page')`
  and `revalidate('all')` render the same tree a navigation to the page would,
  so a form's state, a modal opened from `onSuccess`, a toast in the layout all
  survive. A one-time secret shown after the action is still there.
- **The re-render sees the write.** A `cache()`d read the middleware or the
  action made before the write is forgotten before the page renders again, so
  a renamed row is renamed in the sidebar too, not only in the table.
- **What was fetched before the write is dropped.** Payloads prefetched for
  other links and the pages held behind this one for the back button are from
  before the mutation. A visit to the list after creating a row fetches the
  list, with the row in it, rather than revealing the one prefetched a moment
  earlier. `refresh()` does the same.
- **What comes back is for the page it was invoked on.** A tap that left the
  page while the action was in flight — Add to cart, then the brand link at
  once — changed what is showing, and the trees the host rendered for the
  page before are not put under the url after. The page on screen was
  fetched before the write, so it is asked for again whole, as `refresh('all')`
  would; a modal opened over the page in the meantime does not count as
  leaving it.
