...
Waiting does not hold up the build. A page that has rendered what it can and is
only waiting steps aside for the next one, so hundreds of shells cost about one
budget between them, not one each.
