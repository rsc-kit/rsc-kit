...
of the server render entirely — read it after mount, where there is nothing
to match against:
...
Both renders that have to agree produce the empty string; the clock is read
afterwards, when only the browser is running. The page stays static and the
value is live.

<Aside type="note" title="React is growing an api for this">
  `react-dom` documents [`browser`](https://react.dev/reference/react-dom/browser),
  which does it properly: `use(browser())` inside the component, with the
  closest Suspense boundary's fallback going into the HTML instead of an empty
  first render.

  It is not in 19.2.8 — what an app installs today — so the effect above is the
  version that works now. This package deliberately does not ship its own
  `ClientOnly` in the meantime: it is React's boundary to draw, and a stand-in
