...
For something that is genuinely per-visitor and needs no server, read it after
mount, where there is nothing to match against:
...
The server renders nothing, the browser fills it in, and the page stays
static.
