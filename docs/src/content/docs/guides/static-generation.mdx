...
And moving it into a client component makes it worse, not better. A client
component still renders on the server, so the HTML carries the build's value
and hydration produces a fresh one — the two disagree, and React reports a
hydration mismatch. Measured on a frozen page: the HTML said `13:31:39`, the
browser rendered `13:32:02`.
