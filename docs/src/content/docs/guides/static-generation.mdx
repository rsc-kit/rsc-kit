...
Turn it off when your build machine cannot do what your pages do.
...
build, and a page that calls an API needs its credentials there. A typical CI
container has neither, and a route that throws fails the build.

Failing the build is the right default. It is the wrong thing to be stuck with
when the cause is the build machine rather than the page.
