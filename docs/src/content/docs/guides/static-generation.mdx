...
<Aside type="note" title="Laravel runs this for you">
  `npm run build` bundles and prerenders in one step, writing the frozen
  pages to `bootstrap/rsc/vite/static` — which is where the built server reads them
  from. Nothing in PHP drives it; the build is Vite's.
</Aside>
...
○  /
   ⚠  froze new Date() — a stored page keeps whatever that returned at build
      time. For a value that should differ per visitor, read it through
      something the build can suspend on, such as an rpc() call.
```
...
<Aside type="caution" title="Suspense alone does not fix this">
  ...
  something inside it *suspends* on what the build cannot resolve, which is
  what an `rpc()` call does.
</Aside>

And moving it into a client component makes it worse, not better. Hydration is
not React attaching handlers to markup that is already there — it runs the
component, because running it is the only way React learns what the tree is
supposed to be; the DOM is what it then matches that tree against. So the body
executes twice, once wherever the HTML came from and once in the browser, and a
clock read inside it gives two answers.
...
For something that is genuinely per-visitor and needs no server, keep it out
of the server render entirely:

```tsx
import { ClientOnly } from '@rsc-kit/core/ClientOnly';
...
The fallback goes into the HTML and is what hydration renders too, so the two
agree; the children run only once React is running in the browser. The page
stays static and the value is live.

<Aside type="note" title="Why this is a component and not an effect">
  Setting state in an effect gets there as well, one render later. `ClientOnly`
  uses `useSyncExternalStore`, whose server snapshot is precisely the thing
  being expressed — React renders the fallback on the server, renders it again
  during hydration so they match, and only then switches.

  `react-dom` documents a [`browser`](https://react.dev/reference/react-dom/browser)
  api that does this properly, leaving a real Suspense fallback in the HTML.
  It is not in 19.2, which is what an app installs today.
</Aside>
