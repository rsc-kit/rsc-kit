...
<Aside type="caution" title="WebKit snapshots the whole page">
  ...
  desktop or an Android phone never shows it. Two ways out: put the
  transition on a container that is the viewport's height and scrolls
  inside, so the snapshot is one screen; or leave the transition off on
  WebKit — `CSS.supports('view-transition-name: a') && !/AppleWebKit(?!.*Chrom)/`
  is not enough, since iOS Chrome is WebKit too; test for `navigator.vendor
  === 'Apple Computer, Inc.'`. The port went with off, at 20–40 ms a
  navigation.
</Aside>
