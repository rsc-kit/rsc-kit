...
The browser's default is a 250 ms cross-fade of the whole segment. On a dark
page that reads as a dip — both snapshots at half opacity over a dark
background — which is the "flash" people notice against a router that does
not animate at all. The transition carries a class, `rsc-navigation`, so your
CSS decides:
