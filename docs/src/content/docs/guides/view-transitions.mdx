...
Out of the box the segment cross-fades in place: the browser would also morph
the transition group between the two pages' heights, and on a page-sized
segment that slides below-the-fold content through the viewport, so the
engine holds the group still. What is left is the browser's 250 ms cross-fade.
On a dark page that reads as a dip — both snapshots at half opacity over a
dark background — which is the "flash" people notice against a router that
does not animate at all. The transition carries a class, `rsc-navigation`, so
your CSS decides:

```css title="src/app/styles.css"
/* Keep the old page as it is and only fade the new one in: no dip. */
::view-transition-old(.rsc-navigation) {
  animation: none;
}
::view-transition-new(.rsc-navigation) {
  animation: 120ms ease-out both rsc-fade-in;
}
...
`animation: none` on both is an instant swap — what Inertia does — while
keeping the boundary in place for pages that animate their own parts. Name
the class yourself with `rscKit({ viewTransitions: { className: 'nav' } })`.
