...
changed only the server leaves every open page alone. `RSC_BUILD_VERSION`
in the server's environment overrides it — a commit id there makes every
deploy reload every stale page, which is the stronger guarantee at the
cost of the reloads.
