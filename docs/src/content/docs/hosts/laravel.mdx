...
It writes the map as well, and a dev server that is running starts again
when the map changes — so the export is there to import the moment the
command returns, with nothing to restart by hand. A class you write yourself
is picked up the next time `rsc:action-manifest` runs, which the `dev` and
`build` scripts do first.
