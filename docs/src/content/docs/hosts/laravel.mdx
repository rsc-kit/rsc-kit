...
Two processes. The renderer — what `vite dev` serves, and what Nitro builds
into `.output/server` — owns the request: routing, rendering, the frozen
pages, the assets. Laravel answers what only it can: the data, the session,
and whether a route may render at all.

They speak HTTP. There is no runtime for PHP to launch and no socket protocol
to implement; a server component that writes `await rpc('Orders.recent')`
causes one POST to your application, under the visitor's own cookie, and the
render resumes when the JSON comes back.
...
:::caution[`php artisan serve` cannot host this]
`artisan serve` is `php -S`: one worker, which cannot answer a second request
while it is blocked on the first. When Laravel proxies a page it holds that
worker for the whole render, and the renderer calls back to the same server for
the page's data — with nobody left to answer. The call would time out after
30 seconds into a page that answers **200** with its data missing, because a
failed host call is reported inside its Suspense boundary. The package refuses
that shape up front instead.

Herd, Valet, PHP-FPM and Octane all run several workers. Or skip the proxy:
open the renderer's origin directly and let Laravel answer host calls only.
:::
...
```tsx title="resources/js/rsc/app/orders/page.tsx"
...
```tsx title="resources/js/rsc/app/orders/CancelButton.tsx"
'use client'
...
may render. A `route.ts` beside or above a page names middleware in Laravel's
own vocabulary:

```ts title="resources/js/rsc/app/admin/route.ts"
...
needs. A page with no `route.ts` middleware makes no guard call; sibling
components awaiting `rpc()` are rendered concurrently, so their calls
overlap into one round; `cache()` dedupes identical calls within a request;
a frozen page makes none at all and a shell only for its holes. A guarded
page is typically two rounds — the guard, then the data in parallel — which
is why a host-call-heavy app is better served by Octane.
