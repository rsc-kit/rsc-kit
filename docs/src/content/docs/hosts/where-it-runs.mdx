...
`--bytecode` is the one worth a decision, and it needs its spelling:
`--bytecode --format=esm`. Alone, Bun ignores it for this entry, byte for
byte; with the format said explicitly the JavaScript is compiled ahead of
time. What a production port measured: cold boot to the first healthy
response 275 ms → 72 ms, for an image of 81 MB → 123 MB — on a pod that
takes seconds to schedule, with a startup probe every 2 s, the pull time
cost more than the boot saved, and they left it out. On the example app,
with 3.4 MB of JavaScript, it changed nothing measurable and added 5 MB.
It earns its place on a large app that cold-starts often — a scale-to-zero
service, a preview environment per branch — and nowhere else.
