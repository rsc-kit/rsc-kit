...
the file — and costs readable stack traces from production. `--bytecode`
applies to CommonJS output only; the entry is ESM with a top-level `await`,
so Bun ignores the flag, byte for byte. `--target bun` is the default. Time
to the first answered request was 85 ms with every flag and with none.
