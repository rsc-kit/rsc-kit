...
```bash
npm run compile   # builds, then bun build --compile --keep-names .output/server/compile.mjs
./dist/app
```
...
minifies — and minifying renames every function. A partially prerendered page
is finished by replaying a tree the build recorded by component *name*, so a
binary that renamed them finds the wrong name in every slot:

```text
Expected the resume to render <J> in this slot but instead it rendered <J2>.
```

React then falls back to client rendering for every hole, and the page arrives
looking finished with nothing wired up. `--keep-names` preserves the names the
build recorded, and costs a few kilobytes. A build with no PPR route never
notices; one with them fails only on a cold load, in production, which is the
worst way to find out.

`--target bun` is the default. `--bytecode`
