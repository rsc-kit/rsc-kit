...
An rsc-kit app with a backend is a **BAP — Backend-Answered Pages**: the
page is rendered in front of the backend rather than by it. The renderer
asks; the backend answers. (The table on [the Laravel page](/hosts/laravel/#the-model-a-bap)
puts it beside MPA and SPA.)
