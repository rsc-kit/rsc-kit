...
A scaffolded app has no server file. If you would rather run the renderer inside
an existing server, `createRscHandler` is exported and anything the route table
does not claim comes back `null`, so your own routes still win:

<CodeFromFile file="examples/app/server.ts" title="mounting it by hand" />
...
`build` freezes every page it can, so there is no separate prerender step. To
redo the freezing against fresh data without rebuilding, run
