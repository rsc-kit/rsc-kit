...
`start` runs the entry itself. Bun and Node 24 both execute TypeScript, so there
is nothing to compile first — and bundling it turned out to cost more than it
saved: inlining the build's own output gives React's resume a second copy of
every client component under a renamed binding, which silently disables partial
prerendering.

`compile` produces a standalone binary from that same entry. It is why
`server.ts` imports the build with a static path — a bundler traces static
specifiers to decide what to embed, so a resolved-at-runtime path would leave
the engine out. The binary still reads assets and frozen pages from disk, so
ship `build/` beside it.
