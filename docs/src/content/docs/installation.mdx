...
and there is no reason for them to sit among your own files. `rsc-routes.d.ts`
lists the routes it found, which is what turns `Link`, `redirect()` and friends
into [typed apis](/guides/routing#typed-links); `rsc-env.d.ts` declares the host
global, and `rsc-engine.d.ts` the generated bundle.

Only what has to be. A type you can import is imported —
[`Metadata`](/guides/metadata/) comes from the package, so it works before
anything has been built. What remains is global by nature: a function installed
on `globalThis`, a union generated from your own routes, a module that does not
exist until the build writes it.
