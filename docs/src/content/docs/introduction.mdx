...
`rsc-router` is a file-based router for React Server Components. You write
pages, layouts and server actions the way you would in the Next.js App Router;
it renders them, streams the HTML, and turns links into segment-level SPA
navigations.

It is not a framework. It is a Vite plugin and a request handler, so it runs
wherever a `Request` goes in and a `Response` comes out — Bun, Node, Deno,
Hono, Elysia, Cloudflare Workers — and, through a PHP adapter, inside a Laravel
application.
