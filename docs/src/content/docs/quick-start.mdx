...
export const env = createEnv({
  ...
  clientPrefix: 'VITE_',
  client: {},
  // No bare process.env: import.meta.env carries the VITE_ values, and
  // the file does not throw in the browser if a client component imports it.
...
one has to start with `VITE_`. That is [`@t3-oss/env-core`](https://env.t3.gg),
with the schema in whichever library you chose. `.env.example` is written
beside it and is the one that gets committed.

Import `env` from server code. A client component reads its `VITE_` value
from `import.meta.env.VITE_…` directly: importing `src/env.ts` there brings
the validator along to read one string, about 20 kB gzipped for Zod. The
value is still checked, because the server imports `env` when it starts.
