...
export const env = createEnv({
  ...
  client: {},
  // No bare process.env: import.meta.env carries the PUBLIC_ values, and
...
Import `env` from server code. A client component reads its `PUBLIC_` value
from `import.meta.env.PUBLIC_…` directly: importing `src/env.ts` there brings
