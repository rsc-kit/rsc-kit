...
/**
 * A failure, in the one shape that survives the boundary.
 *
 * Rendered into the payload rather than thrown out of the response, because a
 * batch carries several answers and one of them failing must not take the
 * others with it. Marked so a query whose own result happens to be an object
 * with these keys is not mistaken for a failure.
 */
export const QUERY_ERROR = '__rscQueryError'
...
export function queryError(message: string): QueryError {
  return { [QUERY_ERROR]: true, message }
}

export function isQueryError(value: unknown): value is QueryError {
  return typeof value === 'object' && value !== null && QUERY_ERROR in value
}

/**
 * The most restrictive Cache-Control among a batch's reads.
 *
 * A batch is one response carrying several answers, so it can only be stored
 * as narrowly as its narrowest member. Taking the widest — or the first — is
 * how a public read batched beside a personal one puts someone's data in a
 * shared cache under a url anyone can repeat.
 */
export function narrowestCacheControl(controls: string[]): string {
  if (controls.length === 0 || controls.some((control) => control.includes('no-store'))) {
    return 'private, no-store'
  }

  if (controls.some((control) => control.startsWith('private'))) {
    // max-age is dropped rather than reduced to the smallest in the batch: the
    // reads may have asked for wildly different lifetimes, and revalidating is
    // the only answer that cannot serve one of them stale.
