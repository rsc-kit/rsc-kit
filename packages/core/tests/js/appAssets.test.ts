...
import { appAssets, headTags } from '../../src/appAssets'
...
describe('what the build finds', () => {
  ...
  test('icons, however many, in a stable order', () => {
    const found = appAssets(appWith('icon-512.png', 'icon-192.png', 'icon.svg'))

    expect(found.icons.map((i) => i.file)).toEqual(['icon-192.png', 'icon-512.png', 'icon.svg'])
  })
  ...
  test('share images', () => {
    const found = appAssets(appWith('opengraph-image.png', 'twitter-image.jpg'))
  ...
  test('and nothing at all is not an error', () => {
    ...
    expect(appAssets(join(tmpdir(), 'does-not-exist-at-all')).favicon).toBeNull()
  })
...
describe('what goes in the head', () => {
  ...
  test('share images are absolute when the app said where it lives', () => {
    ...
    // of both.
    const tags = headTags(appAssets(appWith('opengraph-image.png')), 'https://orders.example.com')

    expect(tags).toContainEqual({
      tag: 'meta',
      props: { property: 'og:image', content: 'https://orders.example.com/_app/opengraph-image.png' },
    })
  })
