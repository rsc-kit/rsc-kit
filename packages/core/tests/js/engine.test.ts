...
async function timeline(stream: ReadableStream, markers: string[]) {
  ...
  while (true) {
    ...
    for (const marker of markers) {
      if (!(marker in seen) && buffer.includes(marker))
        seen[marker] = Date.now() - start;
    }
  }
...
function serverActionId(exportName: string): string {
  ...
  throw new Error(
    `no registered server action named "${exportName}" in ${assets}`,
  );
}
...
describe("suspense streaming", () => {
  ...
  test("streams independent boundaries as each resolves", async () => {
    ...
    const seen = await timeline(stream, [
      "Shell rendered",
      "arrived after 500ms",
      "arrived after 3000ms",
    ]);

    expect(seen["Shell rendered"]).toBeLessThan(seen["arrived after 500ms"]);
    expect(seen["arrived after 500ms"]).toBeLessThan(
      seen["arrived after 3000ms"],
    );
  });
  ...
  test("puts <title> in the shell rather than behind the slow boundary", async () => {
    ...
    const seen = await timeline(htmlStream, [
      "<title>",
      "arrived after 3000ms",
    ]);
...
describe("ppr classification", () => {
  ...
  test("leaves the real host callable installed afterwards", async () => {
    await engine.handleRscPprShell(
      "app/page",
      {},
      LAYOUTS,
      ["app/loading"],
...
describe("server actions", () => {
  ...
  test("the bundle says which actions a client built", async () => {
    ...
    const ids = [
      serverActionId("createOrder"),
      serverActionId("greet"),
      "nope#missing",
    ];
    const out = await audit(ids);

    expect(out.find((a) => a.id.endsWith("#createOrder"))).toMatchObject({
      client: true,
      query: false,
    });
    expect(out.find((a) => a.id.endsWith("#greet"))).toMatchObject({
      client: false,
      query: false,
    });
...
describe("loading.tsx validation", () => {
  ...
  async function buildApp(
    ...
    routeConfig = LARAVEL_ROUTE_CONFIG,
  ) {
    ...
    const proc = Bun.spawn(
      ["bun", join(packageRoot, "src/build-rsc-vite.ts")],
      {
        ...
        env: {
          ...
          RSC_VITE_CONFIG: join(
            packageRoot,
            "tests/fixtures/vite.rsc.config.mjs",
          ),
...
describe("form data serialization", () => {
  ...
  async function build(data: Record<string, unknown>) {
    const { buildFormData } = await import(
      join(packageRoot, "src/js/formEncoding.ts")
    );
...
describe("intercept manifest reaches the browser entry", () => {
  function buildApp(withInterceptor: boolean): {
    entry: string;
    cleanup: () => void;
  } {
  ...
  test("patterns the host publishes are baked into the entry", () => {
    ...
    expect(entry).toContain(
      'createViteRscApp(document, [{"urlPattern":"/shop/item/[id]","slot":"modal"}], ',
    );
...
describe("route config is host-supplied", () => {
  test("the same route.php is ignored when no host configures it", async () => {
    ...
    writeFileSync(
      join(dir, "app/dynamic/page.tsx"),
      "export default function P() { return <main>hi</main> }\n",
    );
    writeFileSync(
      join(dir, "app/dynamic/route.php"),
      "<?php\n\nreturn route()->props(fn () => ['a' => 1]);\n",
    );
...
describe("rendering without a client bootstrap", () => {
  ...
  test("leaves out the segment boundary, which is itself a client component", async () => {
    ...
    const withRuntime = await engine.handleRsc(
      "app/static/page",
      {},
      null,
      LAYOUTS,
    ...
    const without = await engine.handleRsc(
      "app/static/page",
      {},
      null,
      LAYOUTS,
  ...
  test("reports the client components that would force a runtime", async () => {
    ...
    const result = await engine.handleRsc(
      "app/page",
      {},
      null,
      LAYOUTS,
  ...
  test("names them, even the ones React sent as a reference to an earlier row", async () => {
    ...
    expect(
      result.clientComponents.some((name: string) => name.startsWith("$")),
    ).toBe(false);
  ...
  test("says whether a server action is in the tree", async () => {
    ...
    const page = await engine.handleRsc(
      "app/page",
      {},
      null,
      LAYOUTS,
...
describe("a section: a named region of a page", () => {
  ...
  test("an unknown name says which names there are", async () => {
    ...
    await expect(engine.handleRscRevalidate("nope", LEDGER)).rejects.toThrow(
      "orders",
    );
  });
...
describe("the urls a route declares", () => {
  test("are reached by calling the page export through the bundle", async () => {
    ...
    expect(await engine.getStaticParams("app/photo/[id]/page")).toEqual([
      { id: "1" },
      { id: "2" },
    ]);
  });
...
describe("the route table the bundle carries", () => {
  test("is the one this build produced", async () => {
    ...
    const onDisk = JSON.parse(
      readFileSync(join(outDir, "routes.json"), "utf-8"),
    );

    expect(
      embedded.routes.map((r: { component: string }) => r.component).sort(),
    ).toEqual(
...
describe("a route guard", () => {
  test("runs even when the caller says it holds every layout", async () => {
    ...
    const attempt = engine.handleRscStream(
      "app/guarded/page",
      {},
      chain,
      [],
  ...
  test("and a revalidation cannot reach past it either", async () => {
    ...
    await expect(engine.handleRscRevalidate("page", page)).rejects.toThrow(
      "Redirect to /login",
    );
  });
  ...
  test("but not the build, which has no request to allow", async () => {
    ...
    const { body } = await engine.handleRsc(
      "app/guarded/page",
      {},
      null,
      chain,
...
describe("a url nothing answers, with a not-found.tsx", () => {
  // Next.js keeps a navigation to a missing route in the app once the page is
  ...
  test("a document request still gets html, through the layout, with the status", async () => {
    const res = await engine.default(new Request("http://x/nope"));
